...
public class PostController {
    ...
    public String postList(@PathVariable Integer id, Integer categoryId,
            @AuthenticationPrincipal LoginUser loginUser, Model model, @PageableDefault(size = 3) Pageable pageable) {

        // SELECT * FROM category WHERE userId = :id
        // 카테고리 가져가세요!!
        PostRespDto postRespDto = postService.게시글카테고리별보기(id, categoryId, pageable);
